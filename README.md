...
designed to utilize [typescript](https://www.typescriptlang.org); [amd (requirejs)](https://requirejs.org); [bootstrap](https://getbootstrap.com);

## introduction
this project is one of two (2) mininum projects required for application development and deployment, and concerns the client-side (front end) implementation of the application.
if you're not familiar with vuejs, we recommend you [get familiar with their framework](https://vuejs.org/v2/guide/).
## getting started
the single page application uses the [`main.ts`](/main.ts) file to 'bootstrap' the application.
...
```
the [`default.html`](/cli/main.html), along with **all** html template files, must have a single root element.
```html
<main>
  <div class="card">
...
```
i usually make all **routable** component, html template files, have a root `<main></main>` element.
